...
function setActiveMenu() {
    ...
    menuItems.forEach(item => {
        ...
        if (link) {
            ...
            if (currentPath.includes('/survey-project-wise') && href.includes('/survey-project-wise')) {
                item.classList.add('active');
            } else if (currentPath === href || (href !== '/' && currentPath.includes(href))) {
                item.classList.add('active');
            }
        }
    });
...
function formatUserName(userData) {
    ...
    if (userData.fullName && userData.fullName !== 'undefined' && userData.fullName !== 'null') {
...
(function() {
    ...
    const isAuthorized = userType && userEmail && 
                         userType !== 'undefined' && 
                         userType !== 'null' && 
                         userEmail !== 'undefined' && 
                         userEmail !== 'null';
...
async function loadProjectSurveys(projectId) {
    ...
    try {
        ...
        if (project && project.surveys) {
            ...
            console.warn(`No project found with ID "${projectId}" or it has no surveys.`);
...
function fetchSurveys(query = "", sortBy = "creation desc") {
    const apiUrl = `${API_BASE}/api/method/leadtech_survey.leadtech_survey.get_all_survey.get_all_doctype_same_format` +
    ...
    fetch(apiUrl, {
        ...
        .then((data) => {
            ...
            if (currentProjectSurveys && Array.isArray(currentProjectSurveys)) {
                ...
                const normalizedProject = currentProjectSurveys.map(normalizeForComparison);
                ...
                surveyData = surveyData.filter(survey => {
                    ...
                    return normalizedProject.includes(normalizedName) || strictProject.includes(strictName);
                });
            ...
            if (!surveyData || surveyData.length === 0) {
                surveyTableBody.innerHTML = '<tr><td colspan="100%">No Surveys available.</td></tr>';
                return;
            }
            let index = 1;
            surveyData.forEach((survey) => {
                ...
                row.innerHTML = `
                    ...
                    <td><i class="fas fa-tasks" onclick="openPopup('${survey.name}')"></i></td>
                `;
